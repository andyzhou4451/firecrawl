...
- [x] **Examples**: [WeChat public account scraper](examples/wechat_public_scraper) *(includes a helper script to test scraping)*
...
An example project for scraping WeChat articles is available in
[examples/wechat_public_scraper](examples/wechat_public_scraper). It demonstrates
how to provide custom headers when calling the Firecrawl API. After setting your
`FIRECRAWL_API_KEY` (and optionally `WECHAT_URL`), you can run
`test_wechat_scraper.py` in that directory to verify the scraper works with your
credentials.
